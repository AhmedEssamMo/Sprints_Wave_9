...
void display_WaitingData()
{
	...
	while(!(PSHBTTN_Status(PORT_C, PIN3)));//Adel
...
uint8_t User_UpdateBalance(void)
{
	...
	EEPROM_WriteBytes(EEPROM_CH_0, MEMORY_BLOCK_4, u8_StartAddress, (ptr_uint8_t) &ST_CurrrentCardDATA.balance,sizeof(f32_t));
	return OperationSuccess;
}